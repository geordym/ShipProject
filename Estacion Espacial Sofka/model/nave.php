...
class nave
{
    ...
    public function insertarNave(Nave $data)
    {
        ...
        if ($data instanceof Lanzadera) {
            ...
            $this->CNX->prepare($query)->execute(array("Robotica", 
            $data->nombre, $data->paisOrigen,
             $data->fechaActividad,
             $data->cantidadEmpuje, $data->cantidadPeso, $data->funcion));
            $last_id = $this->CNX->lastInsertId();
    ...
    public function eliminarNave($id, $tipo_nave)
    {
      
        if(strpos($tipo_nave, 'Robotica') !== false){
         
            $query = "DELETE naves FROM naves
            INNER JOIN roboticas ON naves.id_nave = roboticas.id_nave AND naves.id_nave = $id AND roboticas.id_nave = $id";
   

        }elseif(strpos($tipo_nave, 'Lanzadera') !== false){
        
            $query = "DELETE naves FROM naves
            INNER JOIN lanzaderas ON 
            naves.id_nave = lanzaderas.id_nave 
            AND naves.id_nave = $id 
            AND lanzaderas.id_nave = $id";
            
       
        
        }elseif(strpos($tipo_nave, 'Tripulada') !== false){

            $query = "DELETE naves FROM naves
            INNER JOIN tripuladas
            ON naves.id_nave = tripuladas.id_nave 
            AND naves.id_nave = $id AND tripuladas.id_nave = $id";
